...
- (void)draw {
  auto view = static_cast<GLKView *>(self.window.rootViewController.view);
  [view bindDrawable];
  run(self.app);
}
...
- (void)scene:(UIScene *)scene
  ...
  void *window = (__bridge void *)(self.window);
  self.app = createApp(&window);
  self.window = (__bridge UIWindow *)(window);
  self.window.windowScene = static_cast<UIWindowScene *>(scene);
  [self.window setNeedsDisplay];

  [self performSelector:@selector(draw) withObject:nil afterDelay:0.0];
}
